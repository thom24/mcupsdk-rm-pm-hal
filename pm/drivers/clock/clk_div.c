...
u32 clk_get_div(struct clk *clk)
{
	...
	return div->get_div(clk);
}

sbool clk_div_notify_freq(struct clk *clk, u32 parent_freq_hz,
			  sbool query)
{
	const struct clk_data *clk_data = clk_get_data(clk);
	...
	for (i = div; !found && i < data_div->n + 1UL; i++) {
		if (!drv_div->valid_div || drv_div->valid_div(clk, i)) {
			found = clk_notify_children_freq(clk,
	...
	if (found && !query) {
		...
		clk_notify_children_freq(clk, parent_freq_hz / i, SFALSE);
		if (i < div) {
			/* Frequency going up */
			drv_div->set_div(clk, i);
		}
...
static u32 clk_div_set_freq_dyn_parent(struct clk *clk,
				       ...
				       sbool query, sbool *changed)
{
	...
	for (i = 1UL; (i <= data_div->n) && (p != NULL); i++) {
		...
		if (*changed && !(clk_notify_sibling_freq(clk, parent,
							  new_parent_freq,
							  STRUE))) {
			continue;
		}
	...
	if (best_div && !query) {
		...
		if (old_div != best_div) {
			drv_div->set_div(clk, best_div);
		}

		/* Assign new rate to siblings */
		if (best_changed) {
			clk_notify_sibling_freq(clk, parent,
	...
	__attribute__((unused)))
{
	...
	if (drv_div->valid_div) {
		for (; div > 0UL && !drv_div->valid_div(clk, div); div--) {
			/* Step through loop until valid div is found */
		}

		for (; div1 <= n && !drv_div->valid_div(clk, div1); div1++) {
			/* Step through loop until valid div is found */
		}
	}
...
u32 clk_div_set_freq(struct clk *clk, u32 target_hz,
		     ...
		     sbool query, sbool *changed)
{
	...
	*changed = SFALSE;
	...
	if (!p || !clk_lookup((clk_idx_t) p->clk)) {
		...
		ret = clk_div_set_freq_dyn_parent(clk, target_hz, min_hz,
...
u32 clk_div_get_freq(struct clk *clk)
{
	...
	return parent_freq_hz / clk_get_div(clk);
}
...
s32 clk_div_init(struct clk *clk)
{
	...
	if (data_div->default_div && drv_div->set_div) {
		if (!drv_div->set_div(clk, data_div->default_div)) {
			ret = -EINVAL;
		}
	}
...
sbool clk_div_reg_set_div(struct clk *clk, u32 d)
{
	...
	n = data_div->n;
	if (d <= n && (!drv_div->valid_div || drv_div->valid_div(clk, d))) {
		u32 v;
...
static u32 clk_div_fixed_get_div(struct clk *clk)
{
	const struct clk_data *clk_data = clk_get_data(clk);
	const struct clk_data_div *data_div;
