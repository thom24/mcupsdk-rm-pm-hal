...
#define UDMAP_TCHAN_TFIFO_DEPTH_FDEPTH_MASK        (0x000000FFu)
...
s32 rm_udmap_set_thrd_id(u16 id, u16 index, sbool tx_ch, u32 thread_id)
{
	...
	u32 ch_base;
	u32 thrd_id_reg;
	u32 trace_action = (tx_ch == STRUE) ?
			   TRACE_RM_ACTION_UDMAP_TX_CH_SET_THRD_ID :
			   TRACE_RM_ACTION_UDMAP_RX_CH_SET_THRD_ID;

	rm_trace_sub(trace_action,
		     TRACE_RM_SUB_ACTION_UDMA_CH_THREAD_ID,
		     thread_id);
	...
	if (r == SUCCESS) {
		if (tx_ch) {
			maddr = rm_core_map_region(inst->tchan->base);
			ch_base = maddr + UDMAP_CH_BASE(index);

			thrd_id_reg = rm_fmk(
				UDMAP_TCHAN_THRD_ID_THREAD_ID_SHIFT,
				UDMAP_TCHAN_THRD_ID_THREAD_ID_MASK,
				thread_id);
			if (writel_verified(thrd_id_reg,
					    ch_base + UDMAP_TCHAN_THRD_ID) !=
			    SUCCESS) {
				/* Readback of write failed: halt */
				r = -EFAILVERIFY;
			}
			...
			thrd_id_reg = rm_fmk(
				UDMAP_RCHAN_THRD_ID_THREAD_ID_SHIFT,
				UDMAP_RCHAN_THRD_ID_THREAD_ID_MASK,
				thread_id);
			if (writel_verified(thrd_id_reg,
					    ch_base + UDMAP_RCHAN_THRD_ID) !=
			    SUCCESS) {
				/* Readback of write failed: halt */
				r = -EFAILVERIFY;
			}
